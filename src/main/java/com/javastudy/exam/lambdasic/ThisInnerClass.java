...
public class ThisInnerClass {
    ...
    class Inner {
        ...
        void method2(int arg, int arg2) {
            FunctionalInterfaceVoidMethod fi = (x, y) -> {  // 매개변수없고 리턴도없는 함수형 인터페이스, 람다식으로 구현
                System.out.println("outterField : " + outterField);
                System.out.println("innerField : " + innerField);
                System.out.println("fieldo : " + ThisInnerClass.this.field); //밖의 field, ThisInnerClass.this는 외부의 ThisInnerClass 를 가리킨다.
                System.out.println("fieldi : " + this.field); //로컬의 field, 그냥 this 붙이면 inner의 this가 된다.
                System.out.println("arg : " + arg); //로컬의 field
                System.out.println("x : " + x); //로컬의 field
                System.out.println("y : " + y); //로컬의 field
            };
            fi.method(arg, arg2); // 람다식으로 구현된 함수형 인터페이스 실행. fi.method()
        }
    }
}
